...
#include "stl.h"
#include "uniqsproto_include.h"
#include "uniqsproto_define.h"
...
class UniqsProto {
   ...
   private:
    ...
    bool ReadProto_struct_property(pugi::xml_node& node, UniqsProto_Struct& rStruct, std::string& strError);
   ...
   public:
    // std::vector<包含文件名, 描述>
    std::vector<UniqsProto_Include> vecIncludes;
    // 防止重复包含
    ...
    std::vector<UniqsProto_Struct> vecStructs;
    // 防止重复包含
